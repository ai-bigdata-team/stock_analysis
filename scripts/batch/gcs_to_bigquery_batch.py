...
def load_raw_ohlcv_to_bigquery(spark):
    ...
    try:
        ...
        print(f"⚠️  Could not refresh path (might be first time): {str(e)}")

    try:
        df = spark.read.parquet(RAW_OHLCV_PATH)
    except Exception as e:
        print(f"⚠️  No data found or error reading: {str(e)}")
        return

    if df.rdd.isEmpty():
        print("⚠️  No records found in RAW path. Skipping...")
        return
    ...
    if now.weekday() == 5:  # Saturday
        ...
        print(f"\n🗓️  Today is Saturday, using Friday as end date")
    elif now.weekday() == 6:  # Sunday
        end_date = now - timedelta(days=2)
        print(f"\n🗓️  Today is Sunday, using Friday as end date")
    else:
    ...
    if records_after == 0:
        print(f"⚠️  No records found for date range {start_date_str} to {end_date_str}. Skipping...")
        return
    ...
    try:
        while True:
            ...
            try:
                ...
                print("⚠️  Will retry on next scheduled run...")
